...
В Windows рекомендуется дважды щёлкнуть `START_MOEX_ANALYTICS.bat`. При первом запуске
скрипт создаст `.venv`, проверит Python и все обязательные runtime-зависимости. Если пакет
отсутствует, launcher предложит установить проект командой `python -m pip install -e .`,
повторно проверит окружение и автоматически продолжит запуск dashboard. База
`database/market.duckdb` остаётся локальной; если её нет, панель предложит начальную настройку.
