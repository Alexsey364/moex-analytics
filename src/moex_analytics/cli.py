...
def build_parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser("build-predictive-feature-store")
    ...
    sub.add_parser("distribution-research-status")
    sub.add_parser("run-scenario-research")
...
def main() -> None:
    ...
    if args.command == "discover":
        ...
        with connection() as con:
            ...
            print(json.dumps(result, default=str, ensure_ascii=True))
    elif args.command in {"run-clean-data-relearning", "clean-data-relearning-status"}:
        from .training_quality import clean_relearning_status, run_clean_data_relearning
        ...
        with connection() as con:
            if args.command == "build-baseline-suite":
                ...
                print(row)
    elif args.command in {"build-predictive-feature-store", "predictive-feature-store-status"}:
        from .predictive_features import core as predictive_features

        init_database()
        with connection() as con:
            if args.command == "build-predictive-feature-store":
                print(predictive_features.build_feature_store(con))
            else:
                print(con.execute("SELECT run_id,status,cutoff,rows,features,families "
                    "FROM predictive_feature_runs ORDER BY started_at DESC LIMIT 1").fetchone())
    elif args.command in {"run-statistical-return-models", "statistical-return-model-status"}:
        from .statistical_models import core as statistical_models

        init_database()
        with connection() as con:
            if args.command == "run-statistical-return-models":
                print(statistical_models.run_statistical_models(con))
            else:
        ...
        with connection() as con:
            ...
            print(action(con))
    elif args.command in {"run-distribution-research", "distribution-research-status"}:
        from .distribution_engine import core as distribution_engine

        init_database()
        with connection() as con:
            action = (
                distribution_engine.run_distribution_research
                if args.command == "run-distribution-research"
                else distribution_engine.distribution_status
            )
            print(action(con))
    elif args.command in {"run-scenario-research", "scenario-research-status"}:
        from .scenario_engine import core as scenario_engine

        init_database()
        with connection() as con:
            action = (
                scenario_engine.run_scenario_research
                if args.command == "run-scenario-research"
                else scenario_engine.scenario_status
            )
            print(action(con))
    elif args.command in {"run-timing-research", "timing-research-status"}:
        from .timing_engine import core as timing_engine
        ...
        "run-uncertainty-aware-rank-groups", "uncertainty-aware-rank-groups-status"
    }:
        from .rank_groups import core as rank_groups
        ...
        with connection() as con:
            ...
            print(action(con))
    elif args.command in {"build-news-reaction-memory", "news-reaction-status"}:
        from .news_reaction import core as news_reaction
