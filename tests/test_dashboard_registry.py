...
def test_registry_counts_and_opportunity_renderer_survive_clean_import():
    ...
    assert counts == {"advanced_pages": 163, "basic_pages": 15}
    source = Path("src/moex_analytics/dashboard/app.py").read_text(encoding="utf-8")
    assert '"Decision Forecast Cockpit": decision_forecast_cockpit.render' in source
...
def test_header_prioritizes_market_cutoff_and_moves_load_log_to_history():
    ...
    assert "Технический журнал доступен в расширенном режиме" in source
